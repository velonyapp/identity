...
func wireApp(contextContext context.Context, infoService *info.Service, data *conf.Data, confTransport *conf.Transport, confSecurity *conf.Security, confObservability *conf.Observability, confGateway *conf.Gateway, logger *slog.Logger) (*kratos.App, func(), error) {
	...
	registerAuthHandler := command.NewRegisterAuthHandler(confSecurity, user, session, unitOfWork, accessTokenManager, sessionTokenManager, passwordHasher, cache, usernameAvailability)
	loginAuthHandler := command.NewLoginAuthHandler(confSecurity, user, session, unitOfWork, accessTokenManager, sessionTokenManager, passwordHasher, cache)
